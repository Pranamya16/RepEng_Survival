"""Stage 2 (Kaggle runner) — Llama-3.2-3B-Instruct, Qwen2.5-3B-Instruct,
Phi-4-mini-instruct baseline activation extraction & refusal behavior.
Account: pranamyadeshpande. Runs sequentially in one kernel, entirely
server-side, so it doesn't depend on any local process staying alive.

Gemma-2-2B-IT runs as its own separate kernel (already in progress when
this was pushed). This one picks up the remaining 3 models. Same fixed
logic as kaggle/stage2_baseline_extraction (apply_chat_template fix,
dataset-mounted HF token fallback), just scoped to these 3 models.
"""
...
if hf_token:
    ...
    print("No usable HF_TOKEN; Llama-3.2 is gated and will fail without one.")
...
print("Stage 2 (Kaggle, remaining 3 models) run complete.")
