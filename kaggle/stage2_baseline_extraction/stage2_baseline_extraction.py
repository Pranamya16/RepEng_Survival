"""Stage 2 (Kaggle runner) — baseline activation extraction & refusal behavior.

Same logic as notebooks/02_baseline_extraction.ipynb, adapted for Kaggle
instead of Colab: no Drive mount (outputs go to /kaggle/working/, fetched
afterward via `kaggle kernels output`), no anti-disconnect cell (Kaggle
kernels run server-side once committed, browser doesn't need to stay open).

HF token: Kaggle's Secrets add-on does not carry over to kernels triggered
via `kaggle kernels push` (API), only interactive runs - so the token is
read from a mounted private dataset (pranamyadeshpande/repeng-survival-hf-token)
instead, with Kaggle Secrets tried first in case that ever changes.

Still follows claude.md rule 3: one model on GPU at a time, unloaded before
the next. Gemma-2-2B-IT and Llama-3.2-3B-Instruct are gated on HF - if no
token is available, those two will fail cleanly (caught per-model) while
Qwen2.5-3B-Instruct and Phi-4-mini-instruct still complete.
"""
...
if hf_token:
    ...
    print("No usable HF_TOKEN; gated models (Gemma-2, Llama-3.2) will fail, others will still run.")
...
print("Stage 2 (Kaggle) run complete.")
